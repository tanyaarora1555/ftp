...
void download_file(int sock, const char *filepath) {
    ...
    if (filename) {
        filename++;
        char folder[256];
        strncpy(folder, filepath, filename - filepath);
        folder[filename - filepath - 1] = '\0'; // null-terminate before '/'
        change_directory(sock, folder);
    } else {
        filename = filepath;
    }
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        printf("Usage:\n connect <ip> <user> <pass>\n upload <file>\n connect_upload <ip> <user> <pass> <file>\n connect_download <ip> <user> <pass> <file>\n connect_list <ip> <user> <pass>\n connect_list_folder <ip> <user> <pass> <folder>\n quit\n");
        return 1;
    }
